...
1. **Миграции**: Убедитесь, что на продакшене применены все три миграции в правильном порядке:
   - `66cd825725a2` (English Reading models)
   - `eadf0bc80aff` (Daily Assignments)
   - `1762253286` (image_url для passages)
   
2. **Изображения**: Убедитесь, что папка `static/images/passages/` с 10 изображениями загружена на сервер

3. **Переводы**: Файл `translations/ar.py` должен быть на сервере (2790+ ключей)

4. **Зависимости**: Все зависимости уже в `requirements.txt`, дополнительных установок не требуется

5. **Blueprints**: Убедитесь, что в `app.py` зарегистрированы:
   - `english_bp` (API: `/api/english/*`)
   - `english_reading_bp` (страницы: `/english/*`)

6. **Модели**: Проверьте, что в `models.py` есть:
   - `EnglishPassage` (с полем `image_url`)
   - `EnglishQuestion`
   - `UserEnglishProgress`
   - `DailyAssignment`
   - `MedicalTerm.term_ar` (для арабских терминов)
