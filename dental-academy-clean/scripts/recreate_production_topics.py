#!/usr/bin/env python3
"""
Скрипт для принудительного пересоздания тем в сообществе для продакшена
"""
...
def delete_existing_topics():
    """Удаляет существующие темы с сообщениями"""
    with app.app_context():
        # Удаляем темы с сообщениями
        ...
        db.session.commit()
        print(f"✅ Deleted {deleted_count} topics with messages")
...
def recreate_production_topics():
    """Принудительно пересоздает темы для продакшена"""
    ...
    with app.app_context():
        try:
            ...
            print("🗑️ Удаляем существующие темы...")
            deleted_count = delete_existing_topics()
