#!/usr/bin/env python3
"""
Финальный скрипт для создания форума
Исправляет проблемы с колонками базы данных
"""
...
def create_forum_categories(conn):
    ...
    # Создаем категорию с минимальными полями
    try:
        cursor.execute("""
            INSERT INTO forum_categories (name, description, created_at)
            VALUES (%s, %s, %s)
            RETURNING id
        """, (
            'Collega Chat',
...
def create_forum_topics(conn, category_id):
    ...
    for topic_data in topics_data:
        ...
        # Создаем тему с минимальными полями
        try:
            cursor.execute("""
...
def main():
    """Основная функция"""
    print("🚀 ФИНАЛЬНОЕ СОЗДАНИЕ ФОРУМА")
    print("=" * 60)
